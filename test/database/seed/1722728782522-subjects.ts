...
export class Subjects1722728782522 implements MigrationInterface {
  ...
  private subjects: Partial<SubjectEntity>[] = [
    ...
    {
      private: false,
      createdBy: { name: 'tst_admin' } as UserEntity,
      key: 'tst_dummy1',
      display_name: 'tst_dummy1',
      type: SubjectType.PERSON,
      short_description: 'I am number 1',
      long_description:
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const [{ id: dummyTagId }, { id: goodTagId }, { id: evilTagId }] =
      await queryRunner.manager.save(
        ...
        this.tags.map((tag) => ({
          ...tag,
          createdBy: { id: sysUser.id },
          modifiedBy: { id: sysUser.id },
        })),
      );

    const adminUser = await queryRunner.manager.findOneByOrFail(UserEntity, {
      name: 'tst_admin',
    });
    const normUser = await queryRunner.manager.findOneByOrFail(UserEntity, {
      name: 'tst_user',
    });
    await queryRunner.manager.save(
      SubjectEntity,
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    const subjsToRemove = await queryRunner.manager.findBy(SubjectEntity, {
      createdBy: { name: In(['tst_admin', 'tst_user']) },
    });
    await queryRunner.manager.softRemove(subjsToRemove);

    const tagsToRemove = await queryRunner.manager.findBy(SubjectTagEntity, {
      createdBy: { name: In([SYS_USER, 'tst_admin', 'tst_user']) },
    });
