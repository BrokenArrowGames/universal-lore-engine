...
export class Subjects1720681571222 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
      const SYS_USER = this.config.app.sysUser;
        ...
        const [{id: dummyTagId }, { id: goodTagId }, { id: evilTagId }] = await queryRunner.manager.save(SubjectTagEntity, this.tags.map((tag) => ({ ...tag, createdBy: { id: sysUser.id }, modifiedBy: { id: sysUser.id } })));
        ...
        const tstAdmin = await queryRunner.manager.findOneByOrFail(UserEntity, { name: this.config.user.root.name });
        const tstUser = await queryRunner.manager.findOneByOrFail(UserEntity, { name: this.config.user.test.name });
        await queryRunner.manager.save(SubjectEntity, [
            { createdBy: tstAdmin, modifiedBy: tstAdmin, key: "sys_admin_item", display_name: "Owned by sys admin", type: SubjectType.IDEA, long_description: "this is a description" },
            { createdBy: tstUser, modifiedBy: tstUser, key: "sys_user_item", display_name: "Owned by sys user", type: SubjectType.IDEA, long_description: "this is a description" },
        ]);
    }
