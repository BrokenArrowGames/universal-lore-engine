...
import { AdminCreateUserCommand, AdminSetUserPasswordCommand, CognitoIdentityProviderClient, CreateUserPoolClientCommand, CreateUserPoolCommand, NumberAttributeConstraintsType } from "@aws-sdk/client-cognito-identity-provider";
...
async function RunMigrations(
  ...
  if (hostname)
    process.env.DB_APP_HOST = hostname;
...
async function StartCognito(
  logger: pino.Logger,
  { hostname, port, PoolName }: { hostname: string, port: number, PoolName: string }
) {
  ...
  const { UserPool: { Id: UserPoolId } } = await client.send(new CreateUserPoolCommand({ PoolName }));
  
  const { UserPoolClient: { ClientId: UserPoolClientId } } = await client.send(new CreateUserPoolClientCommand({
    UserPoolId,
  ...
  for (const Username of ["tst_admin", "tst_user"]) {
    ...
    await client.send(new AdminSetUserPasswordCommand({
      UserPoolId, Username, Password: "mysecretpassword"
    }));
  }
